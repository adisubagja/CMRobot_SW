...
const std::string g_cmrJointTypeName[6]={"fixed","continuous","revolute","prismatic","floating","planar" };
...
//! parse link Data
void cmrURDFParser::parseLinkData(const XMLElement* curLinkElement, cmrLinkData& linkData)
{
    ...
    if(visualElement=curLinkElement->FirstChildElement("visual")){
        //! geometry file
        const XMLElement* meshElement= visualElement->FirstChildElement("geometry")->FirstChildElement();
        linkData.m_geometryFilePath=meshElement->FirstAttribute()->Value();

        //! geometry origin
        const XMLElement* originElement=visualElement->FirstChildElement("origin");
        std::istringstream xyz(originElement->FindAttribute("xyz")->Value());
        xyz>>linkData.m_geometryOrigin.pos[0]>>linkData.m_geometryOrigin.pos[1]>>linkData.m_geometryOrigin.pos[2];
    ...
    if(inertialElement=curLinkElement->FirstChildElement("inertial")){
        //! mass center origin
        const XMLElement* originElement=inertialElement->FirstChildElement("origin");
        std::istringstream xyz(originElement->FindAttribute("xyz")->Value());
        std::istringstream rpy(originElement->FindAttribute("rpy")->Value());
        xyz>>linkData.m_massCenterOrigin.pos[0]>>linkData.m_massCenterOrigin.pos[1]>>linkData.m_massCenterOrigin.pos[2];
        double roll, pitch, yaw;
        rpy>> roll>>pitch>>yaw;
        ...
        //! inertia
        const XMLElement* inertiaElement=inertialElement->FirstChildElement("inertia");
        ...
        linkData.m_inertia<<Ixx,Ixy,Ixz,
                            Ixy,Iyy,Iyz,
                            Ixz,Iyz,Izz;
    }else{
...
//! parse joint Data
void cmrURDFParser::parseJointData(const XMLElement* curJointElement, cmrJointData& jointData)
{
    ...
    if(axisElement){
        ...
        xyz>>jointData.m_jointAxis[0]>>jointData.m_jointAxis[1]>>jointData.m_jointAxis[2];
    }else{
    ...
    const XMLElement* parentElement=curJointElement->FirstChildElement("parent");
    ...
    //！ joint origin
    const XMLElement* originElement=curJointElement->FirstChildElement("origin");
    if(originElement){
        std::istringstream xyz(originElement->FindAttribute("xyz")->Value());
        std::istringstream rpy(originElement->FindAttribute("rpy")->Value());
        xyz>>jointData.m_jointOrigin.pos[0]>>jointData.m_jointOrigin.pos[1]>>jointData.m_jointOrigin.pos[2];
        double roll, pitch, yaw;
        rpy>> roll>>pitch>>yaw;
    ...
    if(limitElement){
        jointData.m_maxEffort=std::stod(limitElement->FindAttribute("effort")->Value());
        jointData.m_maxVelocity=std::stod(limitElement->FindAttribute("velocity")->Value());
    ...
    //! joint dynamics
    const XMLElement* dynamicsElement=curJointElement->FirstChildElement("dynamics");
    if(dynamicsElement){
        jointData.m_damping=std::stod(dynamicsElement->FindAttribute("damping")->Value());
        jointData.m_friction=std::stod(dynamicsElement->FindAttribute("friction")->Value());
    }
}
    
//! converte rpy string to rot matrix
void cmrURDFParser::rpyStrToRotMat(const std::string rpyStr, cmrMatrix3d& rotMat)
{
