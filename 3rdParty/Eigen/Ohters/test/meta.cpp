...
EIGEN_DECLARE_TEST(meta) {
  ...
  STATIC_CHECK((!internal::is_convertible<const Matrix3f, Matrix3f &>::value));
  STATIC_CHECK((internal::is_convertible<Matrix3f, Matrix3f &>::
                    value)); // std::is_convertible returns false here though
                             // Matrix3f from; Matrix3f& to = from; is valid.
  // STATIC_CHECK((!internal::is_convertible<Matrix3f,Matrix3d>::value ));
  // //does not work because the conversion is prevented by a static assertion
