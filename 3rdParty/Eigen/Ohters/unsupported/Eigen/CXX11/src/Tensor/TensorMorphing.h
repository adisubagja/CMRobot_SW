...
struct TensorEvaluator<const TensorReshapingOp<NewDimensions, ArgType>,
  ...
  evalSubExprsIfNeeded(EvaluatorPointerType data) {
  ...
  block(OutputTensorBlock *output_block) const {
    if (m_impl.data() != NULL) {
    ...
    const Index output_outer_dim_size =
        output_block_sizes.TotalSize() / output_inner_dim_size;
    const typename TensorEvaluator<ArgType, Device>::Dimensions &input_dims =
        m_impl.dimensions();
      ...
      while (inner_idx < output_inner_dim_size) {
        ...
        for (Index i = 0; i < NumInputDims; ++i) {
          ...
          input_block_sizes[dim] = numext::mini(
              num_to_allocate,
              (static_cast<Index>(input_dims[dim]) - input_coords[dim]));
        ...
        InputTensorBlock input_block(
            index, input_block_sizes, input_block_strides, m_inputStrides,
            output_block->data() + outer_idx * output_inner_dim_size +
                inner_idx);
...
struct TensorEvaluator<const TensorSlicingOp<StartIndices, Sizes, ArgType>,
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
  block(TensorBlock *output_block) const {
    TensorBlock input_block(
        srcCoeff(output_block->first_coeff_index()),
        output_block->block_sizes(), output_block->block_strides(),
        TensorBlockDimensions(m_inputStrides), output_block->data());
    m_impl.block(&input_block);
  }
...
struct TensorEvaluator<TensorSlicingOp<StartIndices, Sizes, ArgType>, Device>
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
  writeBlock(const TensorBlock &block) {
...
struct TensorEvaluator<
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool
  evalSubExprsIfNeeded(EvaluatorPointerType) {
    m_impl.evalSubExprsIfNeeded(NULL);
    return true;
  }
