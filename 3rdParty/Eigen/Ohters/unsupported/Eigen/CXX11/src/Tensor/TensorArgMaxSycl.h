...
public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorTupleReducerDeviceOp(
      ...
      : m_xpr(expr), m_return_dim(return_dim), m_strides(strides),
        m_stride_mod(stride_mod), m_stride_div(stride_div) {}
...
struct TensorEvaluator<const TensorTupleReducerDeviceOp<StrideDims, ArgType>,
  ...
  TensorEvaluator(const XprType &op, const SyclKernelDevice &device)
      : m_impl(op.expression(), device), m_return_dim(op.return_dim()),
        m_strides(op.strides()), m_stride_mod(op.stride_mod()),
