...
__global__ void FullReductionKernel(R, const S, I_,
...
template <int NPT, typename S, typename R, typename I_>
__global__ void InnerReductionKernel(R, const S, I_, I_,
                                     typename S::CoeffReturnType *);

template <int NPT, typename S, typename R, typename I_>
__global__ void OuterReductionKernel(R, const S, I_, I_,
                                     typename S::CoeffReturnType *);
#endif
...
template <typename Self, typename Op> class BlockReducer<Self, Op, true> {
public:
  typedef typename Self::Index Index;
  typedef typename Self::Scalar Scalar;
  typedef typename Self::CoeffReturnType CoeffReturnType;
...
private:
  ...
  template <typename S, typename R, typename I_>
  KERNEL_FRIEND void
  internal::ReductionInitFullReduxKernelHalfFloat(R, const S, I_, half2 *);
  template <int B, int N, typename S, typename R, typename I_>
  KERNEL_FRIEND void internal::FullReductionKernelHalfFloat(R, const S, I_,
                                                            half *, half2 *);
  template <int NPT, typename S, typename R, typename I_>
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
  GetInputCoordsForOutputIndex(Index index,
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void CalculateTargetInputBlockShape(
    ...
    for (int i = 0; i < NumInputDims; ++i) {
      ...
      if (m_reduced[dim]) {
        ...
        coeff_to_allocate =
            numext::maxi(static_cast<Index>(1),
                         coeff_to_allocate / ((*target_input_block_sizes)[dim] *
                                              reducer_overhead));
