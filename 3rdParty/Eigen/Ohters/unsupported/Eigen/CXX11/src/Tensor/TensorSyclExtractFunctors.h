...
template <typename Dim, size_t NumOutputDim> struct DimConstr {
  template <typename InDim> static EIGEN_STRONG_INLINE Dim getDim(InDim dims) {
    return dims;
  }
};
  ...
  struct FunctorExtractor<TensorEvaluator<                                     \
    ...
    FunctorExtractor(const TensorEvaluator<                                    \
        ...
        : m_dimensions(DimConstr<Dimensions, NumOutputDims>::getDim(           \
              expr.dimensions())),                                             \
          m_return_dim(expr.return_dim()), m_strides(expr.strides()),          \
          m_stride_mod(expr.stride_mod()), m_stride_div(expr.stride_div()) {}  \
  };
...
#define SYCLEXTRFUNCTSLICEOP(CVQual)                                           \
  ...
  struct FunctorExtractor<TensorEvaluator<                                     \
    ...
    FunctorExtractor(                                                          \
        ...
        : xprExpr(expr.impl()), m_offsets(expr.startIndices()),                \
          m_dimensions(expr.dimensions()) {}                                   \
  ...
  struct FunctorExtractor<                                                     \
    ...
    FunctorExtractor(const TensorEvaluator<                                    \
                     CVQual TensorStridingSlicingOp<StartIndices, StopIndices, \
                                                    Strides, XprType>,         \
                     Dev> &expr)                                               \
        : xprExpr(expr.impl()), m_startIndices(expr.exprStartIndices()),       \
          m_stopIndices(expr.exprStopIndices()), m_strides(expr.strides()) {}  \
    EIGEN_STRONG_INLINE const StartIndices &startIndices() const {             \
...
#define SYCLEXTRFUNCCHIPPINGOP(CVQual)                                         \
  ...
  struct FunctorExtractor<                                                     \
    ...
    FunctorExtractor(                                                          \
        const TensorEvaluator<CVQual TensorChippingOp<DimId, XprType>, Device> \
            &expr)                                                             \
...
#define SYCLEXTRFUNCIMAGEPATCHOP(CVQual)                                       \
  ...
  struct FunctorExtractor<TensorEvaluator<                                     \
    ...
    FunctorExtractor(const TensorEvaluator<Self, Device> &expr)                \
        : xprExpr(expr.impl()), m_patch_rows(expr.xpr().patch_rows()),         \
...
#define SYCLEXTRFUNCVOLUMEPATCHOP(CVQual)                                      \
  ...
  struct FunctorExtractor<TensorEvaluator<                                     \
    ...
    const typename Self::Scalar m_padding_value;                               \
    FunctorExtractor(const TensorEvaluator<Self, Device> &expr)                \
        : xprExpr(expr.impl()), m_patch_planes(expr.xpr().patch_planes()),     \
