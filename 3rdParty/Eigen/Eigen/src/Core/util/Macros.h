...
// It's likely that MSVC 2013 supports result_of but I couldn't not find a good
// source for that, so let's be conservative.
...
// using either clang or nvcc 7.5 or above
    ...
    (__cplusplus > 199711L && (EIGEN_COMP_CLANG || EIGEN_COMP_NVCC >= 70500))
...
#if EIGEN_MAX_CPP_VER >= 11 &&                                                 \
    (__has_feature(cxx_atomic) || (__cplusplus > 201103L) ||                   \
     ((__cplusplus >= 201103L) &&                                              \
...
#if EIGEN_COMP_MSVC_STRICT && (EIGEN_COMP_MSVC < 1900 || EIGEN_COMP_NVCC) for older MSVC
