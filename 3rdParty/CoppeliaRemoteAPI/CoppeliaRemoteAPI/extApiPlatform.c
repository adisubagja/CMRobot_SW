...
simxShort extApi_endianConversionShort(
    simxShort shortValue) { /* just used for testing purposes. Endianness is
...
simxUShort extApi_endianConversionUShort(
    simxUShort shortValue) { /* just used for testing purposes. Endianness is
                                detected on the server side */
#ifdef ENDIAN_TEST
  simxUShort retV;
  ((char *)&retV)[0] = ((char *)&shortValue)[1];
...
simxInt extApi_endianConversionInt(
    simxInt intValue) { /* just used for testing purposes. Endianness is
                           detected on the server side */
#ifdef ENDIAN_TEST
  simxInt retV;
...
simxFloat extApi_endianConversionFloat(
    simxFloat floatValue) { /* just used for testing purposes. Endianness is
                               detected on the server side */
#ifdef ENDIAN_TEST
  simxFloat retV;
...
simxDouble extApi_endianConversionDouble(
    simxDouble doubleValue) { /* just used for testing purposes. Endianness is
                                 detected on the server side */
#ifdef ENDIAN_TEST
  simxDouble retV;
...
simxVoid extApi_initRand() { srand(extApi_getTimeInMs()); }

simxFloat extApi_rand() { return (((float)rand()) / ((float)RAND_MAX)); }
...
simxFloat extApi_getFloatFromPtr(
    const simxUChar
        *ptr) { /* To avoid BUS ERROR on some processors, when the data is not
                   properly aligned (thanks to Scott Hissam) */
  simxFloat retVal;
  memcpy(&retVal, ptr, sizeof(simxFloat));
  return (retVal);
}

simxInt extApi_getIntFromPtr(
    const simxUChar
        *ptr) { /* To avoid BUS ERROR on some processors, when the data is not
                   properly aligned (thanks to Scott Hissam) */
  simxInt retVal;
...
simxVoid extApi_releaseBuffer(simxUChar *buffer) { free(buffer); }
...
simxVoid extApi_lockResources(simxInt clientID) {
  ...
  if ((GetCurrentThreadId() == _lock1ThreadId[clientID]) &&
...
simxVoid extApi_lockSendStart(simxInt clientID) {
  ...
  if ((GetCurrentThreadId() == _lock2ThreadId[clientID]) &&
...
simxUChar
extApi_launchThread(SIMX_THREAD_RET_TYPE (*startAddress)(simxVoid *)) {
#ifdef _WIN32
...
simxUChar extApi_connectToServer_socket(
    simxInt clientID, const simxChar *theConnectionAddress,
...
simxUChar extApi_connectToServer_sharedMem(
    simxInt clientID, simxInt theConnectionPort) { /* return 1: success */
#ifndef DO_NOT_USE_SHARED_MEMORY
  ...
  if (open_shared_memory(&shared_memory_info)) {
    if (map_shared_memory(&shared_memory_info)) {
      if (shared_memory_info.buffer[0] == 0) {
        set_shared_memory_size(&shared_memory_info,
                               ((simxInt *)(shared_memory_info.buffer + 1))[0]);

        if (map_shared_memory(&shared_memory_info) == false) {
          fprintf(stderr,
                  "Connect (%d): Failed to remap the shared memory \"%s\"\n",
...
simxInt extApi_send_sharedMem(simxInt clientID, const simxUChar *data,
  ...
  while (dataLength > 0) {
    ...
    if (dataLength <=
        _shmInfo[clientID].size) { /* we can send the data in one shot: */
      memcpy(_shmInfo[clientID].buffer + _shmInfo[clientID].header_size,
             data + off, dataLength);
      ((int *)(_shmInfo[clientID].buffer + 6))[0] = dataLength;
      ((int *)(_shmInfo[clientID].buffer + 6))[1] =
          _shmInfo[clientID].header_size;
      ((int *)(_shmInfo[clientID].buffer + 6))[2] = initDataLength;
      dataLength = 0;
    } else { /* just send a smaller part first: */
      memcpy(_shmInfo[clientID].buffer + _shmInfo[clientID].header_size,
             data + off, _shmInfo[clientID].size);
...
simxUChar *extApi_recv_sharedMem(simxInt clientID, simxInt *dataLength) {
  ...
  if (_shmInfo[clientID].buffer[0] != 1) { /* we are not connected anymore */
    return (0);
  }
  ...
  while (retDataOff != totalLength) {
    ...
    /* Tell the other side we have read that part and additional parts could be
     * sent (if present): */
